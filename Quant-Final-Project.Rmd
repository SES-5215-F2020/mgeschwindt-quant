...
```{r recode, message=FALSE, results='hide', echo=FALSE}
...
```{r table 1, message=FALSE, caption="Table 1", render=lemon_print}
head(women_data)
```
...
Once the population and variables were determined and the dataset created, my first method was to understand what the distributions and proportions of this sample are. To do this, I took a different approach depending on whether the variables were continuous or categorical.
...
```{r summary statistics calculation, message=FALSE, results='hide'}
...
```{r summary statistics table, message=FALSE}
...
```{r figure 1, message=FALSE, results='hide'}
ggplot(women_data,
       aes(x=AGEP)) +
...
```{r figure 2, message=FALSE, results='hide'}
ggplot(women_data,
       aes(x=JWMNP)) +
...
```{r figure 3, message=FALSE, results='hide'}
ggplot(women_data,
       aes(x=PINCP)) +
  geom_histogram() +
...
For categorical variables, I calculated each proportion and confidence interval using a T-Test.

Tables 2, 3, and 4 present the proportions and confidence intervals for means of transportation to work, physical disability, and race, respectively.
...
```{r figure 4, message=FALSE, results='hide'}
...
```{r figure 5, message=FALSE, results='hide'}
...
For the two-sample t-tests, age and physically disability, travel time to work and physical disability, and income and ambulatory difficulty were all found to not be significant due to having high p-values.
...
```{r figure 6, message=FALSE, results='hide'}
...
This regression model also reveals that certain variables are more significant than others. For means of transportation to work, the bus, subway, train, and walking are indicated as significant.

### Interactions & Transformations

Finally, I tested out several versions of my initial linear regression model (Model 1) to determine which model is the preferred model. I completed a log transformation of the variable for income for the first model variation (Model 2), because it is possible that it is percentage changes of income which may make a difference for every additional minute for which a woman in NYC commutes to work, rather than discrete dollar amounts. I also attempted to convert age into a categorical variable (Model 3) in order to distinguish between younger women who may have children and therefore longer commute times which consist of dropping children off at school or daycare, and older women who may not have children. Finally, I created two versions of regression models which use an interaction term. One model uses an interaction term for physical disability and age (Model 4), because I suspect that an older woman may be more likely to have a physical disability. The other model uses an interaction term for means of transportation to work and age (Model 5), because I suspect that age may have an affect on what means of transportation a woman is more able to take.

Table 5 presents a comparison between all models.

```{r table 5, message=FALSE}
...
```{r figure 7, message=FALSE, results='hide'}
