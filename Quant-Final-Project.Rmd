...
Table 1 below presents the first 10 observations of the 327 in this dataset:
...
ggplot(women_data,
  ...
  stat_bin(binwidth = 1) +
  labs(title = "Distribution of Age Across Sample Population")
  theme_minimal()
```
...
ggplot(women_data,
  ...
  scale_y_continuous(name = "Number of People in Sample") +
  labs(title = "Distribution of Travel Time Across Sample Population")
  theme_minimal()
```
...
ggplot(women_data,
  ...
  labs(title = "Distribution of Income Across Sample Population")
  theme_minimal()
```
...
```{r table 2, message=FALSE, results='hide'}
...
```{r table 3, message=FALSE, results='hide'}
